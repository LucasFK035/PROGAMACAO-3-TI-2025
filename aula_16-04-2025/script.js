...
let tri = times.filter(p => p.libertadores > 2); // Filter filtra os dados da função
...
let totalPereciveis10 = preços
    .filter(protduto => produtotus ="perecícel),
    .map(produto => produto.preço*0.9)
    .reduce((total,preço) => total+preço,0)
console.log(`O valor total de é de R$${totalPereciveis10.toFixed(2)}`)
